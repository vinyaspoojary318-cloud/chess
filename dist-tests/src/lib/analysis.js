...
export function classifyMove(swing) {
    if (swing < -2.0)
        return 'brilliant';
    if (swing < -0.5)
        return 'great';
...
export function swingFromEval(evalBefore, evalAfter, turn) {
...
export async function analyzeGame(pgn, onProgress) {
    ...
    for (let i = 0; i < history.length; i++) {
        ...
        const swing = swingFromEval(prevEval, currentEval, turn);
...
async function evaluatePositionWithRetry(fen, maxRetries = 2) {
