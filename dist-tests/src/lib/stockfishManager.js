class StockfishManager {
            ...
            (async () => {
                try {
                    ...
                    if (typeof window !== 'undefined' && typeof Worker !== 'undefined') {
                        ...
                        const workerWrapper = {
                            postMessage: (msg) => nativeWorker.postMessage(msg),
                            onmessage: null,
                            onerror: null,
                        ...
                        nativeWorker.onmessage = (e) => {
                            if (workerWrapper.onmessage)
                                workerWrapper.onmessage({ data: e.data });
                        };
                        nativeWorker.onerror = (err) => {
                            if (workerWrapper.onerror)
                                workerWrapper.onerror(err);
                        };
                        ...
                        if (typeof engine.onmessage === 'function') {
                            ...
                            engine.onmessage = (e) => {
                                ...
                                if (workerWrapper.onmessage)
                                    workerWrapper.onmessage({ data: strData });
                                if (origOnMessage && origOnMessage !== engine.onmessage)
                                    origOnMessage(e);
                            };
                        }
                        this.worker = workerWrapper;
    ...
    sendEvalCommand(fen, depth, evalId) {
